    ...
    def join_network(self, bootstrap_node, config):
        if config.get("bootstrap_port") == config.get("port"):
            ...
            if self.successor.id == self.predecessor.id:
                requests.post(f"http://{self.successor.ip}:{self.successor.port}/update_successor/{self.id}/{self.ip}/{self.port}")
            else:
                requests.post(f"http://{self.predecessor.ip}:{self.predecessor.port}/update_successor/{self.id}/{self.ip}/{self.port}")

            # Debo asignarle al bootstrap cual su nuevo antecesor
            requests.post(f"http://{self.successor.ip}:{self.successor.port}/update_predecessor/{self.id}/{self.ip}/{self.port}")
