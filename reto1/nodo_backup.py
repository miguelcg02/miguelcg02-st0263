...
class Node:
    ...
    def runFlask(self):
        app.run(host=IP, port=PORT_FLASK)
    ...
    def start(self):
        ...
        listen_thread = threading.Thread(target=self.listenThread, args=()).start()
        pingSucc = threading.Thread(target=self.pingSucc, args=()).start()
        runFlask = threading.Thread(target=self.runFlask, args=()).start()
    ...
    @app.route('/printFTable', methods=['GET'])
    def printFTable(self):
        self.printFTable()
        return jsonify({"status": "success printFTable"})
    
    @app.route('/printMyInfo', methods=['GET'])
    def printMyInfo(self):
        return jsonify({"status": "success printMyInfo", "id": self.id, "pred": self.predID, "succ": self.succID})
    
    @app.route('/printMyFiles', methods=['GET'])
    def printMyFiles(self):
